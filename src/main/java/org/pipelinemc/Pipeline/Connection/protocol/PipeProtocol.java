...
public abstract class PipeProtocol extends AbstractProtocol {
    ...
    public PipeProtocol(String name, int highestOpcode) {
        super(name);
    ...
    protected <M extends Message, C extends Codec<? super M>, H extends MessageHandler<?, ? super M>> void inbound(int opcode, Class<M> message, Class<C> codec, Class<H> handler) {
        try {
            ...
            Logger.err("Error registering inbound " + opcode + " in " + getName(), e);
        }
    }
    ...
    protected <M extends Message, C extends Codec<? super M>> void outbound(int opcode, Class<M> message, Class<C> codec) {
        try {
            ...
            Logger.err("Error registering outbound " + opcode + " in " + getName(), e);
        }
    }
    ...
    public <M extends Message> MessageHandler<?, M> getMessageHandle(Class<M> clazz) {
        ...
        if (handler == null) {
            Logger.err("No message handler for: " + clazz.getSimpleName() + " in " + getName(),null);
        }
        return handler;
    }
    ...
    public <M extends Message> Codec.CodecRegistration getCodecRegistration(Class<M> clazz) {
        ...
        if (reg == null) {
            Logger.warn("No codec to write: " + clazz.getSimpleName() + " in " + getName());
        }
        return reg;
    }
